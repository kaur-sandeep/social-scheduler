...
function renderThumbnailPreview(input){const holder=input.closest('.video-thumbnail-control'),preview=holder.querySelector('.thumbnail-preview'),file=input.files[0];preview.replaceChildren();preview.classList.toggle('d-none',!file);if(!file)return;preview.style.cssText='display:inline-flex;width:130px;height:62px;flex:0 0 130px;overflow:hidden;border:1px solid #d0d5dd;border-radius:6px;background:#f8fafc';const image=document.createElement('img'),url=URL.createObjectURL(file);urls.push(url);image.src=url;image.alt='Selected video thumbnail';image.className='selected-thumbnail-image';image.style.cssText='display:block;width:130px;height:62px;max-width:none;object-fit:contain';preview.append(image)}
